...
- QUIC deadlines are exposed through `Transport::next_deadline()` and processed by `poll()`; no async runtime or hidden timer thread is used. Idle drivers block on `BlockingTransport::wait_for_input()` (a readiness peek on the UDP socket) instead of polling on a fixed cadence. Both report immediately-due work when events are already buffered, so calls made between polls -- `listen`, `open_stream`, and the stream operations -- never leave a host asleep on an undelivered event.
...
This crate is a concrete transport adapter and depends on `std`. For Sans-I/O contracts and shared types, use `minip2p-transport`.

## Clocks

Unlike the portable transports, this adapter does not run purely on the host's time sample. quiche keeps its own clock: it reads `Instant::now()` internally to drive loss detection and to answer `conn.timeout()`. Converting it to caller-supplied time would mean forking or wrapping quiche's timer handling, which is out of scope, so the dual clock is deliberate and this adapter is `std`-only.

What the adapter _does_ guarantee is that nothing quiche's clock touches leaks into what the host sees:
