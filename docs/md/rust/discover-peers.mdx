...
`DiscoveryEvent` reports peers being discovered, updated, or expired, plus automatic dial failures and rejected beacons. Use `next_discovery_event(deadline)` when a synchronous loop wants one focused event.
...
:::warning Discovery beacons are always required to be signed. Unsigned discovery beacons are rejected even when application pubsub was configured with `allow_unsigned: true`. :::

:::note minip2p owns the discovery topic. Manually subscribing is redundant, and its pubsub messages are consumed before reaching application pubsub events. Calling `unsubscribe` for that topic returns `PubsubError::DiscoveryTopicReserved`. :::
