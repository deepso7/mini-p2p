...
The TypeScript SDK separates _finding a usable network path_ from _opening an application protocol_. Connect first, wait for Identify readiness, then open a negotiated stream.
...
Both peers must support the same ID. Wait for `peerReady` before opening so the remote Identify snapshot is available.
