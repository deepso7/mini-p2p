...
Errors are public classes exported by `@minip2p/node` and `@minip2p/react-native`. Use `instanceof` for recovery and the typed `kind` fields for diagnostics.
...
See the [TypeScript reference](/reference/typescript-api#public-errors) for the public error classes.
