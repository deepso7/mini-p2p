...
`SwarmRuntimeError` carries `peer_id`, `conn_id`, and `stream_id` whenever the corresponding identity is known. In particular, asynchronous outbound multistream and unsupported-protocol failures retain the stream id needed by hosts to correlate an open request. 5. Hand each `SwarmOutput::Event` to the application. 6. Before waiting on I/O again, `core.is_idle()` should be true.
